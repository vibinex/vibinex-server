...
const QuickStartGuide = ({ bitbucket_auth_url, image_name }: { bitbucket_auth_url: string, image_name: string }) => {
	...
	const architecturalDiagram = "https://github.com/Alokit-Innovations/.github/assets/7858932/d5a97883-64ef-498f-b97a-318b6675ac87";
	...
	React.useEffect(() => {
		...
		rudderEventMethods?.track(getAuthUserId(session), "quickstart page", { type: "page", eventStatusFlag: 1, name: getAuthUserName(session) }, anonymousId)

		const handleGitHubAppClick = () => {
			rudderEventMethods?.track(getAuthUserId(session), "Install github app", { type: "link", eventStatusFlag: 1, source: "quickstart", name: getAuthUserName(session) }, anonymousId)
		};

		const handleAuthoriseBitbucketOauthButton = () => {
			rudderEventMethods?.track(getAuthUserId(session), "Authorise bitbucket consumer", { type: "button", eventStatusFlag: 1, source: "quickstart", name: getAuthUserName(session) }, anonymousId)
		};

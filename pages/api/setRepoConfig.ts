...
const setRepoConfigHandler = async (
	...
	if (req.method !== 'POST') {
		...
		rudderStackEvents.track(session?.user?.id ?? "absent", "", 'set-repo-config', { type: 'HTTP-405', eventStatusFlag: 0, eventProperties });
	...
	if (!isSetRepoConfigReqBody(req.body)) {
		...
		rudderStackEvents.track(session?.user?.id ?? "absent", "", 'set-repo-config', { type: 'HTTP-400', eventStatusFlag: 0, eventProperties });

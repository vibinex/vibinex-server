...
const pubsubHandler = async (req: NextApiRequest, res: NextApiResponse) => { // To be removed, only used for testing the functions
    ...
    if (!projectId || !location) {
        ...
        res.status(400).json({"error": "env variables must be set"})
		return;
	}

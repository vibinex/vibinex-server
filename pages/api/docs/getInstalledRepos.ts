...
const getInstalledReposForUser = async (req: NextApiRequest, res: NextApiResponse) => {
	...
	if (!topicId || typeof topicId !== 'string') {
		...
		rudderStackEvents.track("absent", "", 'installed-repos-for-user', { type: 'HTTP-400', eventStatusFlag: 0, eventProperties });
	...
	const userReposFromDb = await getUserRepositoriesByTopic(topicId, provider).catch((err) => {
		...
		const eventProperties = { ...event_properties, response_status: 400 };
		rudderStackEvents.track("absent", "", 'installed-repos-for-user', { type: 'HTTP-500', eventStatusFlag: 0, eventProperties });
	...
	rudderStackEvents.track("absent", "", 'installed-repos-for-user', { type: 'HTTP-200', eventStatusFlag: 1, eventProperties });

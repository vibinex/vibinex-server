...
const getInstalledReposForUser = async (req: NextApiRequest, res: NextApiResponse) => {
	...
	if (method !== 'GET') {
		...
		rudderStackEvents.track("absent", "", 'docs-installed-repos-for-user', { type: 'HTTP-405', eventStatusFlag: 0, eventProperties });
		return res.status(405).json({ error: 'Method not allowed' });
	}
	const session = await getServerSession(req, res, authOptions);
	if (!session) {
		const eventProperties = { response_status: 401 };
		rudderStackEvents.track("absent", "", 'docs-installed-repos-for-user', { type: 'HTTP-401', eventStatusFlag: 0, eventProperties });
	...
	if (!topicId || typeof topicId !== 'string') {
		...
		rudderStackEvents.track("absent", "", 'docs-installed-repos-for-user', { type: 'HTTP-400', eventStatusFlag: 0, eventProperties });
